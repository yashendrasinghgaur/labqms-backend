...
def send_email(to_email: str, subject: str, html_body: str) -> bool:
    """Send an HTML email using SMTP settings from config.json."""
    config = get_config()
    ...
    if not smtp_user or not smtp_pass:
        log.warning("Email not configured in config.json  skipping email send.")
        return False
    ...
    <div style="padding:28px;">
      ...
      <div style="background:#fef3c7;border:1px solid #fbbf24;border-radius:8px;padding:14px;margin:16px 0;">
        <p style="margin:0;color:#92400e;font-size:13px;">
          <strong>Security Note:</strong> Keep your Lab Code and Password safe.
